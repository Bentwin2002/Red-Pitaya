...
module PI_ctrl (
    ...
    assign in1 = $signed(S_AXIS_tdata[13:0]) * gain1 + (offset1 <<< 10);
    assign in2 = $signed(S_AXIS_tdata[16+14-1:16]) * gain2 + (offset2 <<< 10);
    ...
    always @(posedge clk) begin
        ...
        if (int_final >= intLimMax ) begin
            int_final = intLimMax;
        end else if (int_final <= intLimMin) begin
            int_final = intLimMin;
        end
        if (restart_int == 1) begin
            int_store = 0;
        end else begin
            int_store = int_final;
        end
    ...
    always @(posedge clk) begin
        ...
        if ($signed(large_out2) >= 8191) begin
            ...
            out2 <= {large_out2[64-1], large_out2[12:0]};
        end
    end
